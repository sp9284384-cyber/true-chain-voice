...
export function StatusUpdateForm({ reportId, currentStatus, onUpdated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaved(false);
    try {
      await updateReportStatus(reportId, { new_status: selected }, token);
      onUpdated(selected);
      setSaved(true);
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-3">
      ...
      <button
        ...
        className="rounded-card bg-trust px-4 py-2.5 text-sm font-medium text-paper hover:opacity-90 disabled:opacity-50"
      >
