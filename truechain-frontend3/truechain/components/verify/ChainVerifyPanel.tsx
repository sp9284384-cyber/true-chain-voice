    ...
    <div className="rounded-card border border-line bg-surface p-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          className="shrink-0 rounded-card bg-trust px-4 py-2.5 text-sm font-medium text-paper hover:opacity-90 disabled:opacity-60"
        >
      ...
      <div className="mt-6 min-h-[88px]">
        {state === "idle" && <VerifySkeleton />}
        {state === "loading" && <VerifySkeleton />}
        ...
        {state === "loaded" && result && (
          <>
            ...
            <ChainVisualizer
              ...
              brokenAtId={result.verified ? null : result.broken_at_report_id}
            />
          </>
        )}
      </div>
...
function VerifySkeleton() {
  return (
    <div className="animate-pulse space-y-3" aria-hidden="true">
      <div className="h-14 rounded-card bg-surface-raised" />
      <div className="flex gap-2">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-14 w-14 rounded-card bg-surface-raised" />
        ))}
      </div>
    </div>
  );
